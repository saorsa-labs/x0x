...
while [[ $# -gt 0 && $1 != -- ]]; do
  case "$1" in
    --all-features|--no-default-features|--offline|--locked|--frozen) metadata+=("$1") ;;
...
python3 scripts/ci/isolated-runtime.py python3 scripts/ci/nextest-reuse.py run "$scratch" "$@"

...
scope/body mismatch are skipped. It reads and commits independent 256-row
keyset batches, so a failed or interrupted open retains completed projection
work and resumes from the missing rows on the next open. The immutable
existing history hash plus the delete trigger make incremental reconciliation
safe for older writers. Retention and scope purge explicitly remove orphaned
projection rows. This keeps the existing artifact shape and REST fallback
behavior while preventing an invalid artifact from becoming an indexed
canonical hit.

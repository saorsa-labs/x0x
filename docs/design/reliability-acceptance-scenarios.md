...
| ID | Scenario | Steps | Acceptance (PASS) | Fails for |
|---|---|---|---|---|
| G1 | Defer mint until harness machine known | Issue ACP cert; capture matching eligible `GET /owner/agents` row, then invoke durable-owner `GET /owner/placement` before discovery and retain ordered 200/ok captures | Agent absent from successful ledger; pending / unbound — **no** epoch-0 pin to owner machine | **#512** epoch-0 `Pinned(owner)` |

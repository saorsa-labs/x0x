...
**Owner:** Tester
**Product PR:** [#512](https://github.com/saorsa-labs/x0x/pull/512) `fix(discovery): do not pin ACP harnesses to the owner machine`
**Current tip reviewed:** `833bfe86` — **HOLD (P1):** `mode=Acp` fail-open bypasses `PlacementPinned`
...
**Gate:** live dual-daemon smoke is **blocked** until a **corrected** #512 tip exists (not the HOLD head). Offline fixtures and G1–G6 scenarios are ready now. No Ben Mac / production mesh. G3 dual-daemon is **David-only** when the tip is corrected.
...
A narrow correction (David follow-up): **remove the identity-ingest bypass** and **defer minting when the machine is unknown**, with explicit pending semantics. That **prevents new bad records**. It does **not** repair existing ones. The move ceremony is not automatic recovery (source export needs the agent key on the recorded source, which the wrongly pinned owner daemon may never have possessed). Do not replace the bypass with cache deletion or an epoch-0 heuristic. Any repair of existing pins needs a Proposed ADR.
...
| Live single-host dual-daemon (G3) | **Blocked** — wait for a corrected #512 tip that does **not** fail-open `PlacementPinned` on `mode=Acp`. David-only. No Ben Mac. |
| Silent repair of existing bad pins | **Out of scope** (G6). Do not claim it. |

## Scenarios G1–G6

Observe via CLI / REST on **named local instances** (distinct `X0X_HOME`, loopback, shared hermetic `network_id`). Assemble one observational snapshot:

```json
{
  "agent_id": "…",
  "owner_machine_id": "…",
  "placement": {"kind": "pinned|pending|…", "machine_id": "…"|null, "epoch": 0},
  "journal_mode": "acp"|null,
  "harness_machine": {"machine_id": "…", "agent_ids": ["…"]}
}
```

Sources when live is unblocked: `GET /owner/placement` / `GET /owner/agents/:id/placement` (`kind`, `pinned_machine`, `epoch`); identity / health for `owner_machine_id`; `GET /machines/discovered/:id` / `x0x machines get` for `agent_ids`.

| ID | Scenario | Steps (product path) | Acceptance (PASS) | FAIL signature |
|---|---|---|---|---|
| G1 | Defer mint until harness machine known | Issue ACP cert; read placement **before** harness announce/discovery | `kind=pending` (or equivalent unbound); **no** epoch-0 `Pinned(owner)` | Epoch-0 pin to owner machine while harness unknown |
| G2 | After discovery, pin = harness ≠ owner | Harness announces; read placement + `machines get` | `Pinned(harness_machine)`; `harness ≠ owner`; `agent_ids` contains ACP id | Still `Pinned(owner)` and/or `agent_ids: []` |
| G3 | Second local daemon binds agent | David-only: second named instance on same hermetic `network_id`; observe presence / `agent_ids` | Replica machine record lists the ACP id | Replica `agent_ids: []` while QUIC/machine present |
...
- **PASS (0):** `Pinned` to harness machine **≠** owner **and** `harness_machine.agent_ids` contains `agent_id`. **Or** pending/unbound with no harness bind (G1).
- **FAIL (1):** `Pinned` to **owner** with empty / missing `agent_ids` (current hole). **Or** agent bound on a machine that is **not** the pin (wrong-machine / `mode=Acp` fail-open — never PASS).
- **INCONCLUSIVE (3):** missing `placement`, missing required ids, or any other unclassifiable shape.
...
| `pass-pending-unknown-harness` | 0 |
| `fail-pinned-to-owner-empty-agent-ids` | 1 |
| `inconclusive-missing-placement` | 3 |
| `fail-open-wrong-machine-must-not-pass` | 1 |

## Live David-only dual-daemon outline (blocked)

Do **not** run this against `833bfe86` or any HOLD tip. When a corrected tip lands:

1. Two (or three) **named local** instances: `owner`, `harness` (ACP key under a distinct data dir), optional `replica`. Loopback + shared hermetic `network_id`. **Not** Ben Mac, not public discovery.
2. Owner issues ACP cert for the harness public key (`x0x owner agents issue` / `POST /owner/agents/issue`).
3. **G1:** `GET /owner/agents/:id/placement` **before** the harness is discovered — expect pending / no mint, not `Pinned(owner_machine)`.
4. Start harness daemon; announce. **G2:** pin `machine_id` = harness ≠ owner; `x0x machines get <harness>` `agent_ids` contains the ACP id.
...
Feed a hand-built snapshot to the oracle with `--classify-snapshot <file>` (same schema as the fixtures). Live env fetch is not implemented while the tip is HOLD.

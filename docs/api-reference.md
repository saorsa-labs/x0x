...
the `secure/encrypt`, `secure/decrypt`, `secure/reseal` family) refuse with the
body below — and so does `DELETE /history?scope=group:<ID>` (ADR-0066 §3a: the
purge destroys the forensic record, while history **reads** stay open and are
annotated instead — see

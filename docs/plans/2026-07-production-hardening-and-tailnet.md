...
**Implementation notes (completed #122).** The `mpsc::unbounded_channel` was replaced with `mpsc::channel(WS_OUTBOUND_CAPACITY=1024)`. Two pure feeder-policy helpers (`feed_droppable` for topic/control/error frames; `feed_critical` for DM/keepalive) make the drop-vs-close decision daemon-free and unit-testable. A per-session `CancellationToken` (counted-once via an `AtomicBool` guard) drives the slow-consumer close: the DM/keepalive feeder cancels it on a full queue, the writer task races it against BOTH `recv()` and each `ws_tx.send()` (a writer stuck flushing to a stalled socket is the case a recv-only check misses) and, on cancel, sends a WS `Close(1013)` with a 2s flush budget then exits. The writer loop is extracted into `run_ws_writer<S: Sink<Message>>` for deterministic testing.

**DM-inbox verdict (the pinned condition).** The WS direct path uses `Agent::subscribe_direct()`, **not** `dm_inbox`. `subscribe_direct()` registers a bounded `DirectSubscriberQueue` (`src/direct.rs`, capacity `DIRECT_SUBSCRIBER_BUFFER=8192`, `push_drop_oldest`) — so WS DMs are **best-effort / fire-and-forget**, not retained for replay from a per-recipient inbox. Therefore DM/keepalive full → close 1013 is the correct fail-loud behavior (the plan's fallback clause).

**Integration-test finding.** The literal OS/TCP stalled-reader integration test was found **not viable** and was not committed. The per-session outbound mpsc queue never fills via any external flood (REST `/publish` or WS write-half): the multi-layer `gossip → broadcast(256) → outbound(1024) → writer → TCP` buffering means the queue only fills under sustained TCP backpressure, which an external flood cannot reliably induce (empirically: 1500-frame flood delivered only 16 frames to the queue, `ws_outbound_dropped` stayed 0 over 50s). CI-gating coverage is therefore the **11 deterministic unit tests**: 9 feeder-policy tests (drop vs close, count-once, closed-channel) + 2 extracted-writer tests (slow-close-during-idle sends 1013; slow-close-while-blocked-flushing abandons the send and sends 1013). A dedicated stalled-reader harness (e.g. a test-only capacity override or a peer-driven DM flood) is deferred to a follow-up.

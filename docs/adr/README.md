...
- [ADR 0066: Ordinary-Group Fork Anchors and Data-Plane Quarantine Coverage](./0066-ordinary-group-fork-anchors-and-data-plane-quarantine-coverage.md) (proposed 2026-09-18) — completes the ADR 0064 §§2/3 residuals for #732/#472: a normative 24-path data-plane coverage map (12 gated today, 11 ungated, 1 deliberately ungated) with a `file:line` anchor and disposition per path. Ordinary (non-owner-axis) groups receive the persistent marker with `no_anchor: true` and a manual-clear-only exit — the census found `named_groups.rs:3733` returns early for them, so **0 of 24 paths are gated for that population** despite ADR 0064's text. Delegation grant/authorize/send-as, registry indexing, history purge, task mutations and signed-public bootstrap publication fail closed; history and delegation reads are annotated, never refused; the lifecycle epoch becomes a compound `(state_revision, quarantine_generation)` token re-checked under the persist lock. No automated eviction, no quorum anchor, no fleet propagation; #639 and #646 not absorbed. Five open questions await David's ratification

...
- [ADR 0061: Duplicate-Home Retirement Is Gated on Provable Emptiness](./0061-duplicate-home-retirement-is-gated-on-provable-emptiness.md) (proposed 2026-09-06) — fills the retirement gap ADR 0060 deferred (#449 P4): a losing device retires its duplicate Home automatically ONLY when it is seated in the canonical Home and the duplicate is provably empty (sole member, no pending joins/invites, no durable history, no group-scoped task lists, no rider grant); join-then-retire ordering, fail-closed probes, and anything else kept and reported via `GET /home` `duplicates[]` with per-duplicate `blockers`. Withdrawal cleans crypto only, so history — and the delegations that live only in history — would otherwise be silently orphaned. Forced retirement of a non-empty duplicate and its purge/repoint work remain open

...
2. **Acyclic move records, predecessor-linked.** An owner-signed `MoveAuthorization {agent_id, move_epoch, from, to, placement}` (no envelope digest — that would be circular); the source machine signs an `ExportReceipt {auth_hash, envelope_digest}` after sealing. Verification is two rules over one fold: **participants** (source/target/owner, holding the full log) accept a record iff signatures verify AND `prev` equals their head (CAS) — forks keep first-valid, drop-and-alert; **mesh peers** never see pre-activation records, so they accept a carried bundle on whole-record owner signature + cross-field coherence + **placement**-epoch monotonicity, while its **cumulative tombstones union in unconditionally** — no head needed, and out-of-order arrival loses no historical revocation.
3. **One durable record; everything else derived.** The signed per-agent log is the only durable state, and it folds **totally** — for every legal shape (initial mint, mid-move, post-activation, post-abort) — to `custodian(A)` (who may sign: mint machine → nobody during transfer → bundle's `to_machine` → `from_machine` restored on abort), grow-only `retired_bindings`, and current `placement`. Key possession is a gate INPUT, not log state: `may_sign = holds_key ∧ custodian==this machine`; quiesce/quarantine are labels of that conjunction. No ordered mutations exist to crash between, replay is idempotent, partial application is impossible by construction. `AbortRecord` is its own signed rollback terminator, legal from any pre-activation head. Activation = verifying and durably storing ONE self-contained owner-signed `ActivationBundle {prev, authorization (embedded canonical MoveAuthorization), retired_bindings (cumulative), placement_record, agent_certificate}` (the certificate and authorization ride inside — shipped authority verification rejects issuer-revocations without the cert, `src/revocation.rs:359-376`, and mesh coherence references in-record fields only) and publishing it on `x0x.move.activation.v1`; the target may sign exactly when its local fold's custodian is it and it holds the key. Every crash state IS the log contents and every re-entry IS a re-read. A daemon-wide `AgentSigningGate` evaluates `may_sign` for every production signing path (`/agent/sign`, `src/server/routes/identity.rs:905`; DM/ACK; forward; gossip contexts): **at most one** live signer at every instant — zero during transfer, exactly one after completion or abort — for cooperating daemons; an offline stolen copy still forges valid signatures and is only defeated at machine-context gates.
4. **Binding revocation, carriers split.** `RevokedSubject` gains `AgentMachineBinding {agent, machine, move_epoch}` — revokes the pair, not the machine; owner-key issued via the issuer-revocation rule extended (`src/revocation.rs:189`); self-revocation is impossible for a two-id subject. **Move tombstones ride inside `ActivationBundle`s** on the activation topic; **ad-hoc tombstones** (owner revoking a binding outside a move) ride `x0x.revocation.v2`; the v1 wire (`src/lib.rs:484`) keeps publishing Agent/Machine records byte-identical, because today's v1 batch is one whole-`Vec` (`src/lib.rs:2953`, `:7689`) that an unknown variant would poison entirely for old peers — so the coarse machine-revocation backstop keeps working for them. Tombstones are exempt from the 90-day sweep: permanent, grow-only.
5. **Placement ledger + enforcement.** Owner-signed `PlacementRecord {agent_id, placement: Pinned(MachineId)|Roaming, placement_epoch}` — new owner-side state; the per-agent log's genesis `PlacementMint` (epoch 0) is minted from the cert journal roster (`GET /owner/agents`, `src/server/routes/profile.rs:150`) and designates ≥1 agent `Roaming`, reconciling ADR 0038's ≥1-Roaming Home invariant with the all-Pinned default; current placement is the last placement-bearing record. Digests ride `MachineAnnouncementV3`; records fetch via a kind-tagged blob protocol v2 (the cert blob path stays v1, `src/announce_blob.rs:54`). Enforcement per `(agent, machine)` **pairing** — binding check `is_binding_revoked` plus placement check `placement_epoch >= max_revoked_binding_epoch(agent)` (equality is the coherent activation case: one bundle mints tombstone and successor record at the same epoch; only strictly older records are stale; a forged equal-epoch pin fails the owner signature) — at: `gate_peer_outbound` (`src/lib.rs:10497`) and `gate_peer_machine_inbound` (`src/lib.rs:10557`) for streams/lanes; the gossip-DM gate after attestation (`src/dm_inbox.rs:1231`, `:1302`); the direct-QUIC DM gate (`src/lib.rs:10313`); the forward mid-flight re-check (`src/forward.rs:758`) over **both** machines' pairings during the transition window; and announce ingest (`src/lib.rs:7952`), which also rejects a pinned agent announcing from a non-pinned machine when a qualifying record is cached. Pre-0043 peers fail open, documented; capability adverts flag them during a move.
...
- E2E: crash recovery IS a log re-read — for every legal log shape (initial mint, mid-move, post-activation, post-abort) the fold's `custodian`/`retired_bindings`/`placement` match the spec table, with **at most** one live signer at every sampled instant (exactly one after completion or abort; the initial mint custodian signs; the source signs again after abort) and torn-tail appends discarded; after Activate, source traffic drops at every enforcing gate — including both machines' pairings during the transition window and a pinned agent announcing from a non-pinned machine — while co-resident agents pass; a mesh peer fed epoch 2 before epoch 1 still enforces epoch 1's retired binding; v1-only peers still receive and enforce Agent/Machine revocations; binding records survive the 91-day sweep; placement mint always yields ≥1 Roaming.
- Unit: participant CAS rejects forks and replays; the fold is total — every legal shape yields defined values, key-possession supplied as an input covers held-but-not-custodian and custodian-but-key-deleted; mesh rule applies monotonicity to placement only while cumulative tombstones union in every arrival order; cross-field coherence drops a record whole when any clause fails against the EMBEDDED authorization (swapped cert, mismatched machines/epoch, non-cumulative set, foreign placement, pin ≠ target); authorization/receipt acyclicity; binding authority matrix (owner yes; agent, machine, third user no); blob-v2 kind framing incl. Bundle fetch; epoch comparisons with **equal** epochs enforcing and strictly-older records ignored.

...
2. **Acyclic move records, predecessor-linked.** An owner-signed `MoveAuthorization {agent_id, move_epoch, from, to, placement}` (no envelope digest — that would be circular); the source machine signs an `ExportReceipt {auth_hash, envelope_digest}` after sealing. Verification is two rules over one derivation: **participants** (source/target/owner, holding the full log) accept a record iff signatures verify AND `prev` equals their head (CAS) — forks keep first-valid, drop-and-alert; **mesh peers** never see pre-activation records, so they accept a carried bundle on whole-record owner signature + cross-field coherence + epoch monotonicity — no head needed.
3. **One durable record; everything else derived.** The signed per-agent log is the only durable state. Tombstone set, current placement, source quiesce, and target quarantine are pure functions of the log evaluated at read time — there are no ordered mutations to crash between, replay is trivially idempotent, and partial application is impossible by construction. `Authorized → Sealed → Imported → Activated → SourceRetired`; `AbortRecord` is its own signed rollback terminator, legal from any pre-activation head, deriving "nothing happened" (epoch burned). Activation = verifying and durably storing ONE owner-signed `ActivationBundle {prev, auth_hash, binding_revocation, placement_record, agent_certificate}` (the certificate rides inside — shipped authority verification rejects issuer-revocations without it, `src/revocation.rs:359-376`) and publishing it on `x0x.move.activation.v1`; the target may sign as the agent exactly when its local log verifies a coherent bundle naming it — un-quarantine IS verification, no separate transition. Every crash state IS the log contents and every re-entry IS a re-read. A daemon-wide `AgentSigningGate` evaluates the same derivations for every production signing path (`/agent/sign`, `src/server/routes/identity.rs:905`; DM/ACK; forward; gossip contexts): **at most one** live signer at every instant — zero during transfer, exactly one after completion or abort — for cooperating daemons; an offline stolen copy still forges valid signatures and is only defeated at machine-context gates.
...
- E2E: crash recovery IS a log re-read — for every log shape the derived `quiesced`/`quarantined`/`live-signer`/`tombstone`/`placement` values match the spec table, with **at most** one live signer at every sampled instant (exactly one after completion or abort) and torn-tail appends discarded; after Activate, source traffic drops at every enforcing gate — including both machines' pairings during the transition window and a pinned agent announcing from a non-pinned machine — while co-resident agents pass; v1-only peers still receive and enforce Agent/Machine revocations; binding records survive the 91-day sweep; placement mint always yields ≥1 Roaming.
- Unit: participant CAS rejects forks and replays; mesh rule accepts higher-epoch bundles, no-ops on equal digest, drops lower epochs, without any head; cross-field coherence drops a record whole when any clause fails (swapped cert, mismatched machines/epoch, foreign placement, wrong auth_hash); abort derives "nothing happened" from every pre-activation head; authorization/receipt acyclicity; binding authority matrix (owner yes; agent, machine, third user no); blob-v2 kind framing; epoch comparisons with **equal** epochs enforcing and strictly-older records ignored.

...
This installs the `x0xd` daemon binary to `~/.local/bin`, starts it, and waits for the health check to pass. The installer verifies the archive GPG signature when GPG is available; without GPG it warns and continues.

On first run, x0xd generates a post-quantum keypair (stored in `~/.x0x/`), connects to bootstrap nodes, and starts the REST API on `127.0.0.1:12700`.

For full installation details including flags, see [install.md](https://x0x.md/docs/install.md).
...
Expected: `{"ok": true, "status": "healthy", "version": "0.4.0", "peers": N, "uptime_secs": N}`
...
Version **0.4.0**. x0x is functional but early-stage. Use it if the current capabilities match your needs — do not rely on planned features.

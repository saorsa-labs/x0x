...
- x0x is a local daemon (`x0xd`) plus peer-to-peer gossip transport for agent-to-agent messaging.

In practice, they are complementary:

- Use MCP when an agent needs structured access to external tools, APIs, or files.
- Use x0x when agents need direct communication, shared trust state, and decentralized coordination.
- A single agent can use both at the same time.
...
- x0x is a transport/runtime layer for encrypted, signed, peer-to-peer gossip messaging.

Practical difference:

- With A2A alone, you still need infrastructure and transport decisions.
- With x0x, agents communicate through local `x0xd` daemons and peer connectivity, without a central broker.

x0x can still participate in A2A-oriented ecosystems by serving an agent card (`.well-known/agent.json`) for discovery metadata.
...
- signed pub/sub message flow exposed through REST (`/publish`, `/subscribe`, `/events`)
- local trust-state management (`/contacts`, `/contacts/trust`)
- shared CRDT-style task list operations (`/task-lists` endpoints)
- local-first operation through a daemon API on `127.0.0.1`

Tradeoff:

- x0x adds a runtime dependency (`x0xd` must be installed and running).
...
- Your runtime is browser-only (no browser-hosted `x0xd` support today).

...
6. **CRDT** (`crdt/`): Collaborative task lists with OR-Set checkboxes (Empty/Claimed/Done), LWW-Register metadata, LWW-Register ordering (no RGA). Deltas of a group-scoped list (`x0x.group.<gid>.symphony.<lid>`) in an `MlsEncrypted` group are sealed with the group's current GSS/TreeKEM key, like group KV stores (`crdt/sealed.rs`, #895); personal lists and `SignedPublic` groups stay plaintext.
...
x0xd [OPTIONS]
  ...
  --relay                         Be a relay node: Full gossip participation AND relay/coordinator advertisement (ADR-0034; also X0X_RELAY_OPT_IN=1 or gossip.relay=true)

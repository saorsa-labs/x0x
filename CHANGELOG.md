...
- **Hidden-group local discovery.** Withdrawing a Hidden group no longer
  ...
  observation with a public tombstone positive control (#509).

- **ACP owner-machine pin prevention.** New ACP placements wait for a
  nonzero discovered harness machine instead of falling back to the owner
  daemon's machine. Normal pairing-gate enforcement is preserved; existing
  incorrect pins are not rewritten or repaired (#512).

- **ACP placement acceptance ledger oracle.** Offline acceptance tooling
  requires a successful `GET /owner/placement` capture and checks pinned
  snapshots against ledger machine and epoch evidence. Missing, malformed
  or contradictory evidence is classified as inconclusive. The G1–G6
  procedure and regression fixtures do not establish live acceptance or
  implement recovery of existing incorrect pins (#524).

- **Identity-ingest placement pin enforcement.** Acceptance and regression coverage
  exercise placement pins through real identity ingestion rather than stubbed
  discovery alone, so pin enforcement is proven on the ingest path (#526).

- **Pending placement requires issuance evidence.** Offline pending-acceptance
  tooling now requires issued ACP evidence before treating a missing pin as a
  pass; absence alone is no longer sufficient (#527).

...
- **Mitigated gossip-DM machine-revocation claim bypass ([#184](https://github.com/saorsa-labs/x0x/issues/184); protocol closure tracked by [#213](https://github.com/saorsa-labs/x0x/issues/213)).** The inbox now retains the freshest authenticated `AgentId → MachineId` binding from accepted verified identity announcements, rejects signed-envelope machine claims that disagree with that binding, and checks revocation against the authenticated machine. The in-memory binding cache is capped at 65,536 entries with LRU eviction and logs every eviction. **KNOWN LIMITATION:** this mitigates but does not fully close the HIGH finding. A sender with no prior accepted announcement still uses the sender-controlled claimed-machine best-effort check; restart empties the non-persistent cache until announcements repopulate it; and an LRU-evicted binding likewise degrades to claim fallback. In those states, a holder of the agent key can claim an unrevoked machine. Issue #213 tracks fresh machine-key attestation bound into gossip DMs plus an explicit portable-agent move policy.

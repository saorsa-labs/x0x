...
|---|---|---|
| [#446](https://github.com/saorsa-labs/x0x/issues/446) | Short-lived session tokens are accepted on some owner-act surfaces (`/agent/sign`, `/exec/*`, `/shutdown`, sync enrollment, delegation creation, `/home/rename`, `/announce`) that are specified durable-owner-only. | Use the durable token for owner actions; treat session tokens as sensitive as the durable token. |

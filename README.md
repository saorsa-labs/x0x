...
|---|---|---|
| [#446](https://github.com/saorsa-labs/x0x/issues/446) — **fixed** | Owner-act surfaces (`/agent/sign`, `/exec/*`, `/shutdown`, `/upgrade/apply`, sync enrollment, delegation creation, `/home/rename` + Home-targeted `PATCH /groups/:id`, `/announce` with user identity, reserved exec payloads via `/direct/send` / WS `send_direct`) require the durable API token, enforced at the route layer before body extraction; session/rider bearers get a typed `403`. Plain `/announce` and non-Home group PATCHes stay session-allowed. | None: the boundary is enforced; the GUI prompts for the durable token on first owner act (tab-scoped `sessionStorage`, never a URL). |

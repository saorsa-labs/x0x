...
class InstallShContractTests(unittest.TestCase):
    ...
    def test_success_json_escaping(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            plat = detect_platform_tag()
            release_dir = create_release_fixture(root, plat)

            install_dir = root / 'install"dir\\path'
            bin_dir = root / 'bin"dir\\path'
            version = '1.2.3\nline\tcol\rreturn"quote\\slash'
            ...
            self.assertEqual(payload["skill_path"], str(install_dir / "SKILL.md"))

#!/usr/bin/env bash
# Phase-B dogfood smoke — groups + contacts via direct DMs.
#
# Boots alice + bob + charlie, exchanges agent cards, starts the Phase-A
# runner on each daemon (subscribed to the discover topic + /direct/events),
# then drives all assertions through tests/e2e_dogfood_groups.py via
# direct DMs from alice (the anchor) to bob and charlie.
#
# Every assertion in this harness is the result of an x0x DM round-trip:
# orchestrator (alice's identity) → runner DM → daemon REST call locally
# → result DM back. No SSH, no curl-from-orchestrator-to-remote-daemon.

set -uo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
X0XD="${X0XD:-$ROOT/target/release/x0xd}"
RUNNER="$ROOT/tests/runners/x0x_test_runner.py"
ORCHESTRATOR="$ROOT/tests/e2e_dogfood_groups.py"

if [ ! -x "$X0XD" ]; then
    echo "x0xd not built at $X0XD — run: cargo build --release --bin x0xd" >&2
    exit 2
fi

NODES=(alice bob charlie)
declare -A API_PORTS=([alice]=24700 [bob]=24701 [charlie]=24702)
declare -a DAEMON_PIDS=()
declare -a RUNNER_PIDS=()

WORK_DIR=$(mktemp -d -t x0x-dogfood-groups.XXXXXX)
echo "work_dir=$WORK_DIR"

cleanup() {
    for p in "${RUNNER_PIDS[@]:-}"; do
        kill -9 "$p" 2>/dev/null || true
    done
    for p in "${DAEMON_PIDS[@]:-}"; do
        kill -TERM "$p" 2>/dev/null || true
    done
    sleep 1
    for p in "${DAEMON_PIDS[@]:-}"; do
        kill -9 "$p" 2>/dev/null || true
    done
}
trap cleanup EXIT

token_path() {
    echo "$HOME/Library/Application Support/x0x-$1/api-token"
}

# 1. Boot daemons.
for node in "${NODES[@]}"; do
    "$X0XD" \
        --name "$node" \
        --api-port "${API_PORTS[$node]}" \
        --no-hard-coded-bootstrap \
        > "$WORK_DIR/$node.x0xd.log" 2>&1 &
    DAEMON_PIDS+=("$!")
    echo "started daemon $node pid=$! port=${API_PORTS[$node]}"
done

echo "waiting for daemons to bind /health..."
for node in "${NODES[@]}"; do
    until curl -sf -m 2 "http://127.0.0.1:${API_PORTS[$node]}/health" \
        > /dev/null 2>&1; do
        sleep 1
    done
done

# 2. Collect tokens + agent ids + cards.
declare -A TOKENS AIDS CARDS
for node in "${NODES[@]}"; do
    tk_file="$(token_path "$node")"
    until [ -s "$tk_file" ]; do sleep 1; done
    TOKENS[$node]="$(cat "$tk_file")"
    port="${API_PORTS[$node]}"
    AIDS[$node]="$(curl -sf -H "Authorization: Bearer ${TOKENS[$node]}" \
        "http://127.0.0.1:$port/agent" \
        | python3 -c "import sys,json;print(json.load(sys.stdin)['agent_id'])")"
    CARDS[$node]="$(curl -sf -H "Authorization: Bearer ${TOKENS[$node]}" \
        "http://127.0.0.1:$port/agent/card")"
    echo "$node aid=${AIDS[$node]:0:16}…"
done

# 3. Cross-import cards so each daemon knows the others.
for src in "${NODES[@]}"; do
    for dst in "${NODES[@]}"; do
        [ "$src" = "$dst" ] && continue
        curl -sf -X POST \
            -H "Authorization: Bearer ${TOKENS[$src]}" \
            -H "Content-Type: application/json" \
            -d "${CARDS[$dst]}" \
            "http://127.0.0.1:${API_PORTS[$src]}/agent/card/import" \
            > /dev/null
    done
done
echo "card exchange complete"

# 4. Direct-connect alice→bob and alice→charlie so the DM hot path is warm.
for peer in bob charlie; do
    curl -sf -X POST \
        -H "Authorization: Bearer ${TOKENS[alice]}" \
        -H "Content-Type: application/json" \
        -d "{\"agent_id\":\"${AIDS[$peer]}\"}" \
        "http://127.0.0.1:${API_PORTS[alice]}/connect" > /dev/null || true
done
sleep 2

# 5. Boot one Phase-A runner per daemon.
for node in "${NODES[@]}"; do
    NODE_NAME="$node" \
    X0X_API_BASE="http://127.0.0.1:${API_PORTS[$node]}" \
    X0X_API_TOKEN="${TOKENS[$node]}" \
        python3 "$RUNNER" \
        > "$WORK_DIR/$node.runner.log" 2>&1 &
    RUNNER_PIDS+=("$!")
    echo "started runner $node pid=$!"
done

sleep 3

# 6. Drive the dogfood scenarios via direct DMs.
echo "==== running dogfood orchestrator ===="
python3 "$ORCHESTRATOR" \
    --api-base "http://127.0.0.1:${API_PORTS[alice]}" \
    --api-token "${TOKENS[alice]}" \
    --anchor alice \
    --runner "bob:${AIDS[bob]}" \
    --runner "charlie:${AIDS[charlie]}" \
    --report "$WORK_DIR/dogfood-groups.json" \
    --cmd-timeout 25
RC=$?
echo "==== orchestrator exit=$RC ===="

if [ $RC -ne 0 ]; then
    for node in "${NODES[@]}"; do
        echo "--- runner log: $node (last 30) ---"
        tail -30 "$WORK_DIR/$node.runner.log"
    done
fi

exit $RC

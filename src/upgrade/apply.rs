...
use super::notification::ReleaseNotification;
use super::signature::verify_bytes_signature_with_key;
...
impl AutoApplyUpgrader {
    ...
    /// Apply an upgrade from a `ReleaseNotification`.
    ///
    /// 1. Find the platform-appropriate asset
    /// 2. Download archive + signature
    /// 3. Verify ML-DSA-65 signature on archive
    /// 4. Extract binary from archive
    /// 5. Replace current binary with backup/rollback
    /// 6. Trigger restart
    pub async fn apply_upgrade_from_notification(
        &self,
        notification: &ReleaseNotification,
    ) -> Result<UpgradeResult, UpgradeError> {
        let current_version_str = crate::VERSION;
        let target_version = semver::Version::parse(&notification.version)
        ...
        let platform_target = super::notification::current_platform_target()
            .ok_or(UpgradeError::UnsupportedPlatform)?;

        let asset = notification
            .matches_platform(platform_target)
            .ok_or_else(|| {
                UpgradeError::Other(format!(
                    "no asset for platform {platform_target} in notification"
                ))
            })?;
        ...
        let result = self
            .download_verify_extract_replace(&upgrader, asset, &temp_dir, &target_version)
            .await;
        ...
        if matches!(upgrade_result, UpgradeResult::Success { .. }) {
            info!(
    ...
    /// Apply an upgrade from an `UpgradeInfo` (GitHub fallback path).
    pub async fn apply_upgrade_from_info(
        &self,
        info: &super::UpgradeInfo,
    ) -> Result<UpgradeResult, UpgradeError> {
        let current_version_str = crate::VERSION;
        let target_version = semver::Version::parse(&info.version)
            .map_err(|e| UpgradeError::Other(format!("invalid version: {e}")))?;
        let current_version = semver::Version::parse(current_version_str)
            .map_err(|e| UpgradeError::Other(format!("invalid current version: {e}")))?;
        ...
        // Download archive
        info!(binary_name = %self.binary_name, "Downloading {} archive...", self.binary_name);
        download_to_file(&info.download_url, &archive_path).await?;

        // Download signature
        info!("Downloading signature...");
        download_to_file(&info.signature_url, &sig_path).await?;
        ...
        if let Err(e) = verify_bytes_signature_with_key(
            &archive_data,
            &sig_data,
            super::signature::RELEASE_SIGNING_KEY,
        ) {
        ...
        extract_binary_from_archive(&archive_path, &extracted_path, &self.binary_name)?;
    ...
    async fn download_verify_extract_replace(
        &self,
        upgrader: &Upgrader,
        asset: &super::notification::PlatformAsset,
        temp_dir: &Path,
        ...
        // Verify ML-DSA-65 signature on archive (before extraction)
        info!("Verifying ML-DSA signature on archive...");
        let archive_data =
            std::fs::read(&archive_path).map_err(|e| UpgradeError::Other(e.to_string()))?;
        let sig_data = std::fs::read(&sig_path).map_err(|e| UpgradeError::Other(e.to_string()))?;
        ...
        upgrader.perform_upgrade(&extracted_path, target_version)
    }

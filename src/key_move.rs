...
impl PlacementRecord {
    ...
    pub fn owner_matches_cert(&self, cert: &AgentCertificate) -> bool {
        // Review r6 H8: the certificate must VERIFY — a forged or
        // tampered certificate must not vouch for a placement.
        cert.verify().is_ok()
...
impl<'a> PlacementAuthority<'a> {
    /// Authority from a certificate: VERIFIES the certificate first — a
    /// forged or tampered certificate cannot mint authority.
    ///
    /// # Errors
    ///
    /// Returns [`IdentityError::CertificateVerification`] when the
    ...
    /// Authority from an `ActivationBundle` carrying the placement:
    /// re-runs the full §7.5 coherence verification — the whole-record
    /// owner signature AND the embedded certificate's issuer binding
    /// prove this owner key owns the agent.
    ///
    /// # Errors
    ///
    /// Returns [`IdentityError::Revocation`] when the chained record is
    ...
    /// The mint's own trust root: the LOCAL owner keypair this daemon
...
impl MoveState {
    ...
    pub fn ingest_bundle(
        ...
        if let Some(placement) = acceptance.placement {
            ...
            if self.cache_placement(placement, PlacementAuthority::bundle_owner(chained)?)? {
                changed = true;
            }
        }
    ...
    pub fn placements_from_bytes(
        ...
        for record in list {
            let authority = certs.get(&record.agent_id).and_then(|cert| {
                PlacementAuthority::cert_issuer(cert)
                    .ok()
                    .filter(|_| record.owner_matches_cert(cert))
            });
    ...
    /// Merge a loaded companion store into this state (the split files
    /// load into one state). Logs/bundles keep the richer side.
    ///
    /// Review r6 H8: incoming PLACEMENTS re-prove authority at this
    /// boundary — a record enters only when its owner key matches the
    /// issuer of a VERIFIED certificate for the agent (from `certs`) or
    /// the issuer already proven by a coherence-verified bundle held in
    /// `other`. A self-signed victim record is REJECTED here.
    pub fn merge_loaded(&mut self, other: Self, certs: &HashMap<AgentId, AgentCertificate>) {
        // Review r6 H8: capture bundle-owner authority BEFORE the maps
        // are consumed — {(agent, owner key)} for every coherence-verified
        // bundle in `other`.
        ...
        for (agent, record) in other.placements {
            let cert_ok = certs
                .get(&agent)
                .is_some_and(|cert| cert.verify().is_ok() && record.owner_matches_cert(cert));
            let bundle_ok = bundle_owners
                .get(&agent)
                .is_some_and(|owner| *owner == record.owner_public_key);
            if !cert_ok && !bundle_ok {
                tracing::warn!(
                    agent = %hex::encode(agent.as_bytes()),
                    "merge_loaded: dropping placement without a proven authority (self-signed or foreign issuer)"
                );
                continue;
            }
...
/// bundles + placements, so a crash can never strand the ceremony between
/// per-file states.
const MOVE_STATE_FILE_MAGIC: &[u8; 4] = b"X0M3";

impl MoveState {
    /// Encode the whole state (logs + bundles + placements + a monotonic
    /// generation) as ONE atomically-writable file — the transactional
    /// commit point for the ADR-0043 ceremony state.
    ///
    /// # Errors
    ///
    /// Returns [`IdentityError::Serialization`] on encode failure.
    pub fn combined_to_bytes(&self, generation: u64) -> Result<Vec<u8>> {
        let body = PersistedMoveState {
            generation,
            logs: self.logs.iter().map(|(a, r)| (*a, r.clone())).collect(),
            bundles: self.bundles.iter().map(|(a, c)| (*a, c.clone())).collect(),
            placements: self.placements.values().cloned().collect(),
        };
        let body = bincode::serialize(&body)
            .map_err(|e| IdentityError::Serialization(format!("move-state.bin encode: {e}")))?;
        let mut out = Vec::with_capacity(MOVE_STATE_FILE_MAGIC.len() + body.len());
        out.extend_from_slice(MOVE_STATE_FILE_MAGIC);
        out.extend_from_slice(&body);
        Ok(out)
    }

    /// Decode `move-state.bin`, re-verifying every log chain and every
    /// bundle under the coherence rule (untrusted disk: a component that
    /// fails verification drops fail-closed). Returns the state and its
    ...
    pub fn combined_from_bytes(
        ...
        {
            return Err(IdentityError::Serialization(
                "move-state.bin missing X0M3 magic".to_string(),
            ));
        }
        let persisted: PersistedMoveState =
            bincode::deserialize(&bytes[MOVE_STATE_FILE_MAGIC.len()..])
                .map_err(|e| IdentityError::Serialization(format!("move-state.bin decode: {e}")))?;
        let mut state = Self::new();
        for (agent, records) in persisted.logs {
            if verify_chain(&records, None).is_ok() {
                state.logs.insert(agent, records);
            } else {
                tracing::warn!(
                    agent = %hex::encode(agent.as_bytes()),
                    "move-state.bin: dropping unverified log on load"
                );
            }
        }
        for (agent, chained) in persisted.bundles {
            if verify_bundle_coherence_chained(&chained).is_ok() {
                if let MoveRecord::ActivationBundle {
                    retired_bindings,
                    placement_record,
                ...
                } = &chained.record
                {
                    ...
                    let _ = state.cache_placement(
                        ...
                        PlacementAuthority::bundle_owner(&chained)?,
                    );
                }
                state.bundles.insert(agent, chained);
            } else {
                tracing::warn!(
                    agent = %hex::encode(agent.as_bytes()),
                    "move-state.bin: dropping incoherent bundle on load"
                );
            }
        }
    ...
    /// Review r3 H8: drop standalone placement records whose owner key
    /// does not match a known certificate issuer (applied by the builder
    /// after `combined_from_bytes`, whose standalone entries have no
    /// in-file authority proof).
    pub fn retain_placements_issued_by(&mut self, certs: &HashMap<AgentId, AgentCertificate>) {
        self.placements.retain(|agent, record| {
            let ok = certs
                .get(agent)
            ...
            if !ok {
                tracing::warn!(
                    agent = %hex::encode(agent.as_bytes()),
                    "dropping standalone placement without a cert-issuer match"
                );
            }
            ok
        });
...
struct PersistedMoveState {
    generation: u64,
    logs: Vec<(AgentId, Vec<ChainedRecord>)>,
    bundles: Vec<(AgentId, ChainedRecord)>,
    placements: Vec<PlacementRecord>,
}

/// Verify a standalone receipt VARIANT's machine countersignature against
/// the machine the authorization names (review r3 C1: the owner's
/// receipt-ingestion path must re-verify what the operator carried).
///
/// # Errors
///
/// Returns [`IdentityError::Revocation`] when the variant is not a
/// receipt or the countersignature is invalid/foreign.
pub fn verify_receipt_variant(
    variant: &MoveRecord,
    expected_machine: &MachineId,
    expected_auth_hash: &[u8; 32],
) -> std::result::Result<(), IdentityError> {
    let carrier = ChainedRecord {
        prev: [0u8; 32],
        record: variant.clone(),
        owner_public_key: Vec::new(),
        owner_signature: Vec::new(),
    };
    let auth_hash = variant
        .auth_hash()
        .ok_or_else(|| IdentityError::Revocation("not a receipt variant".to_string()))?;
    if &auth_hash != expected_auth_hash {
        return Err(IdentityError::Revocation(
            "receipt does not reference the expected authorization".to_string(),
        ));
    }
    carrier.verify_machine_countersignature(expected_machine)
}
...
mod tests {
    ...
    fn move_state_persistence_roundtrip() {
        ...
        loaded.merge_loaded(bundles_state, &load_certs);
        loaded.merge_loaded(placements_state, &load_certs);
...
mod r4_tests {
    ...
    /// WHY (review r4 H8): `cache_placement` with a KNOWN authoritative
    /// owner must refuse a record signed by any other key — signature
    /// validity alone proves internal consistency, not authority. This is
    /// the injection hazard blob-v2 would wire remotely; it must be closed
    /// in the API itself before that transport exists.
    #[test]
    fn cache_placement_requires_expected_owner_authority() {
        let owner = crate::identity::UserKeypair::generate().unwrap();
        let attacker = crate::identity::UserKeypair::generate().unwrap();
        let agent = crate::identity::AgentId([0x11; 32]);
        let good = PlacementRecord::sign(
            agent,
        ...
        let forged = PlacementRecord::sign(
            agent,
            attacker.public_key().as_bytes(),
            Placement::Pinned(crate::identity::MachineId([9; 32])),
            4,
            2,
            attacker.secret_key(),
        )
...
mod r4_egress_tests {
    ...
    /// WHY (review r4 H5): the outbound DM egress B/P check must fire for
    /// a machine resolved from ANY source — here the discovery cache names
    /// a machine the (owner-signed, correctly-issued) placement pins
    /// ELSEWHERE; the send must be refused before any envelope exists,
    /// whether or not a network is configured (the raw-QUIC/capability
    /// resolution paths feed the same `enforce_pairing` predicate).
    #[tokio::test]
    async fn dm_egress_refuses_pinned_elsewhere_recipient() {
        ...
        // A peer agent the daemon has "discovered" on machine OLD.
        let owner = crate::identity::UserKeypair::generate().unwrap();
        let peer = crate::identity::AgentKeypair::generate().unwrap();
        let old_machine = crate::identity::MachineId([0x0A; 32]);
        let new_machine = crate::identity::MachineId([0x0B; 32]);
        agent
            .insert_discovered_agent_for_testing(crate::DiscoveredAgent {
                self_name: None,
        ...
        // The owner pins the peer to NEW — the discovery-cached OLD
        // pairing is stale; P must refuse a DM to it (epoch ≥ no
        // tombstone ⇒ vacuously current).
        let pinned = PlacementRecord::sign(
            peer.agent_id(),
            owner.public_key().as_bytes(),
            Placement::Pinned(new_machine),
            1,
            1,
            owner.secret_key(),
        )
        .unwrap();
        {
            ...
            state
                .cache_placement(pinned, PlacementAuthority::local_owner(&owner))
                .unwrap();
        }
        ...
        let err = agent
            .send_direct(
                &peer.agent_id(),
                b"must not reach the stale pairing".to_vec(),
            )
            .await
...
mod r5_tests {
    ...
    fn combined_load_requires_authority_for_standalone_placements() {
        ...
        .unwrap();
        state
            .cache_placement(minted, PlacementAuthority::cert_issuer(&cert).unwrap())
            .unwrap();
        ...
        .unwrap();
        state
            .cache_placement(orphan, PlacementAuthority::local_owner(&orphan_owner))
            .unwrap();
...
mod r6_tests {
    ...
    /// WHY (review r6 H8): `merge_loaded` re-proves authority at the merge
    /// boundary — a self-consistent but SELF-SIGNED victim record (owner
    /// key that is not the agent's certified issuer, with no proven
    /// bundle) must be REJECTED, while the cert-issuer-backed record
    /// merges.
    #[test]
    fn merge_loaded_rejects_self_signed_victim_placement() {
        let owner = crate::identity::UserKeypair::generate().unwrap();
        let agent_kp = crate::identity::AgentKeypair::generate().unwrap();
        let cert = crate::identity::AgentCertificate::issue(&owner, &agent_kp).unwrap();
        let agent = agent_kp.agent_id();

        // The LEGITIMATE record: owner-signed, cert-issuer-backed.
        let legit = PlacementRecord::sign(
            agent,
            owner.public_key().as_bytes(),
            Placement::Roaming,
            1,
        ...
        // The VICTIM record for a second agent: self-signed by an
        // attacker key (internally consistent — verifies standalone),
        // with NO certificate and NO bundle proving that key owns the
        // agent.
        let attacker = crate::identity::UserKeypair::generate().unwrap();
        let victim_agent = crate::identity::AgentId([0x44; 32]);
        let victim = PlacementRecord::sign(
            victim_agent,
            attacker.public_key().as_bytes(),
            Placement::Pinned(crate::identity::MachineId([0x45; 32])),
            5,
            1,
            attacker.secret_key(),
        )
        .unwrap();
        assert!(victim.verify().is_ok(), "victim is internally consistent");

        // Build the source state the way the loader does (authority-gated
        // writes; the attacker record enters via the LOCAL owner trust
        // root of a hostile/compromised store, simulating untrusted disk).
        let mut other = MoveState::new();
        other
            .cache_placement(legit, PlacementAuthority::cert_issuer(&cert).unwrap())
            .unwrap();
        other
            .cache_placement(victim, PlacementAuthority::local_owner(&attacker))
            .unwrap();

        let mut merged = MoveState::new();
        let mut certs = HashMap::new();
        certs.insert(agent, cert);
        merged.merge_loaded(other, &certs);

        assert!(
            merged.placement(&agent).is_some(),
            "cert-issuer-backed record merges"
        );
        assert!(

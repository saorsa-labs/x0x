...
use routes::{
    ...
    apply_group_state_commits, apply_named_group_metadata_event,
    ...
    force_dm_capability_miss, forward_add, forward_list, forward_remove, get_a2a_agent_card,
    get_agent_card, get_constitution, get_constitution_json, get_group_card,
    get_group_public_messages, get_group_state, get_group_state_commits, get_kv_value,
    get_mls_group, get_named_group, get_named_group_members, gossip_diagnostics,
    group_membership_lock, groups_diagnostics, handle_file_message,
    handle_group_public_typed_payload, handle_join_result_message, handle_public_group_bootstrap,
    ...
    identity_revocations, identity_revoke, import_agent_card, import_group_card, introduction,
    ...
    redeliver_group_public_message, reject_join_request, remove_mls_member,
...
pub async fn serve_with_options(
    ...
    if let Some(secs) = config.dm_capability_cache_ttl_secs {
        builder = builder.with_capability_cache_ttl(std::time::Duration::from_secs(secs.max(1)));
    }
    ...
    let state = Arc::new(AppState {
        ...
        dm_capability_test_controls: config.dm_capability_test_controls,
        connect_diagnostics,
        forward_service,
    });
    ...
    // Strict v2 signed-public bootstrap listener. Unlike the legacy direct
    // fan-out above, this route resolves the authenticated DM completion only
    // after consent validation and a directory-durable install.
    {
        let bootstrap_state = Arc::clone(&state);
        bg_tasks.push(tokio::spawn(async move {
    ...
    // delivers it to active members.
    {
        ...
        bg_tasks.push(tokio::spawn(async move {
            while let Some(typed) = group_public_dm_rx.recv().await {
                handle_group_public_typed_payload(&public_dm_state, typed).await;
            }
        }));
    }
    ...
    // Durable signed-public bootstrap retry worker. The outbox was loaded and
    // reconciled before listener startup; each step sends at most one due
    // obligation and persists exponential backoff, so disconnected peers do
    // not create a hot loop and restart resumes the exact canonical payload.
    {
        let bootstrap_state = Arc::clone(&state);
        bg_tasks.push(tokio::spawn(async move {
    ...
    let app = Router::new()
        ...
        .route(
            "/groups/:id/messages/:msg_id/redeliver",
            post(redeliver_group_public_message),
        )
        ...
        .route(
            "/groups/:id/state/commits/apply",
            post(apply_group_state_commits),
        )
        ...
        .route(
            "/diagnostics/dm/capabilities/:agent_id/force-miss",
            post(force_dm_capability_miss),
        )

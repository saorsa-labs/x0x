...
mod tests {
    use super::*;
    use crate::identity::{MachineKeypair, AgentKeypair};
    ...
    async fn test_keypair_serialization_roundtrip() {
        ...
        assert_eq!(original.public_key().as_bytes(), deserialized.public_key().as_bytes());
        ...
        assert_eq!(original_agent.public_key().as_bytes(), deserialized_agent.public_key().as_bytes());
    }
    ...
    async fn load_machine_keypair_from_path(path: &Path) -> Result<MachineKeypair> {
        let bytes = fs::read(path).await.map_err(|e| IdentityError::Storage(e))?;
        deserialize_machine_keypair(&bytes)
    }

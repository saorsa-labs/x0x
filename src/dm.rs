...
/// delivery. Durable retries recover the accepted logical binding from history
/// across daemon restarts; only an exact protocol/payload/thread match is
/// re-ACKed without another local dispatch.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum DmAckOutcome {
    /// Envelope decrypted, signature verified, and trust-policy passed. Under
    /// v1 it was enqueued to the recipient handler. Under durable v2+ its
    /// protocol, application payload, and thread binding were committed to
    /// history and local dispatch completed.
    Accepted,

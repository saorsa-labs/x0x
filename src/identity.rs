...
impl MachineKeypair {
    ...
    pub fn from_bytes(
        ...
        let secret_key = MlDsaSecretKey::from_bytes(secret_key_bytes)
            .map_err(|_| crate::error::IdentityError::InvalidSecretKey(
                "failed to parse secret key".to_string(),
            ))?;
        Ok(Self {
            public_key,
            secret_key,
...
mod tests {
    ...
    fn test_keypair_generation() {
        ...
        assert!(machine_kp.public_key().as_bytes().len() > 0);
        assert!(agent_kp.public_key().as_bytes().len() > 0);
    }
